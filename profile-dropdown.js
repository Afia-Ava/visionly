...
function setupProfileDropdown(user) {
    ...
    const profileHTML = `
        <div class="profile-container">
            <div class="profile-button" id="profile-button">
                <img src="${user.photoURL || 'https://via.placeholder.com/32x32/ff6b6b/ffffff?text=' + (user.displayName ? user.displayName.charAt(0) : 'U')}" 
    ...
    // Find the profile element in navbar with multiple selectors
    ...
    // If no profile element found, try to find any image in navigation area
    if (!profileElement) {
        const navImages = document.querySelectorAll('nav img, header img, .navbar img, .navigation img');
        if (navImages.length > 0) {
            profileElement = navImages[navImages.length - 1]; // Take the last image
            console.log('Using last navigation image as profile element');
        }
    }
    
    // If still no element found, create one in the navigation
    if (!profileElement) {
        console.log('No profile element found, creating one');
        ...
        if (nav) {
            ...
            console.error('No navigation element found to add profile dropdown');
            return;
        }
    }
...
function setupProfileEvents() {
    ...
    if (!profileButton || !profileDropdown) {
        console.error('Profile elements not found after setup');
        return;
    }
